...
% protocol running% This is the main program used for data segmentation.
% In all protocols, time is divided into 2.5 second long "trials", each
% marked by trial start and trial end digital markers. In three protocols -
% G1, G2 and M2, a stimulus is presented after 1.25 seconds. In the
% remaining trials, nothing happens.
...
% protocol running
% 
function segmentAndSaveData(subjectName,expDate,...
    folderSourceString,~,subjectIndex,problematicIndices)
...
timeStartFromBaseLine = -5; deltaT = 10;
...
for i=1:length(protocolNameList)
    ...
    folderName = fullfile(folderSourceString,'Data', 'rawData', subjectName,gridType,expDate,protocolName);
    ...
    goodStimCodeNums = [];
% ---------------------------------------------------------
% Remove first 2 and last 2 trials ONLY for problematic indices
% ---------------------------------------------------------

if ismember(subjectIndex,problematicIndices)

    if length(goodStimTimes) > 4

        goodStimTimes   = goodStimTimes(3:end-2);
        goodStimTimesML = goodStimTimesML(3:end-2);

    else
        error('Not enough trials to remove first 2 and last 2.');
    end
