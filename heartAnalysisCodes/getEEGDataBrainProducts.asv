% This program extracts ECG data from the raw (.eeg, .vhdr, .vmrk) files
...
if (cAnalog>0)
    ...
    %totalStim = length(analysisOnsetTimes);
    %goodStimPos = round(analysisOnsetTimes * Fs); %two lines added here, converts sec into samples

    
    for i=1:length(analogInputNums)  %1:cAnalog
     ...
     %This loop is specifically for the 'elec number 66 specified
